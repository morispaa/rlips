...
##	A.data	Theory matrix data. Given as a matrix OR 
##			as a vector (row-major format)
##	M.data	Measurement matrix data. Given as a matrix OR 
##			as a row-major vector
##	E.data	Measurement error covariance matrix, OR 
##			vector giving error variances OR
##			single number giving constant error variance 
##			for all measurements.
##			Default is E.data==1, i.e. errors do not have 
...
{ 
	...
	if (is.vector(M.data))
	# If given as a vector, check that its length 
	# is right (given the rows in A.data)
	{
	...
	if (Emat)
	# If errors are given as a covariance matrix, 
	# check its dimensions.
	# NB: Positive definitiveness or symmetricity 
	# is not checked!
	{
		...
		# Multiply theory matrix and measurement data
		# with the inverse of the Cholesky factor of 
	...
	{
		if (length(E.data)==1)
		# If error variance is given as a single number, 
		# expand it to a vector.
		{
	...
	{
		# First e$ncols columns of the buffer contains 
		# the theory matrix and the next e$nrhs columns 
